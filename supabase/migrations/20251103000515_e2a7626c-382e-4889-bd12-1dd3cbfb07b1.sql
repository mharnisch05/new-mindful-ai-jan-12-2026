...
-- Fix 2: Drop ALL existing messages RLS policies
DO $$
BEGIN
  ...
  DROP POLICY IF EXISTS "Therapists can send to own clients" ON messages;
  DROP POLICY IF EXISTS "Clients can send to linked professionals" ON messages;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Service role can read admin overrides" ON admin_overrides;
  DROP POLICY IF EXISTS "Users can view own admin override" ON admin_overrides;
END $$;
...
DO $$
BEGIN
  DROP POLICY IF EXISTS "Service role can insert validation attempts" ON code_validation_attempts;
  DROP POLICY IF EXISTS "Service role can read validation attempts" ON code_validation_attempts;
END $$;
